...
两个 VNH 门锁电机均无霍尔传感器，控制板也未为其提供 ADC 电流采集，因此不具备位置、速度与电流反馈能力，行程控制与超时保护以运行时间作为主要判据。
...
- `motor_bdc_vnh`：面向 VNH 类 PWM + 双方向引脚电机的实现；此类电机无位置、速度与电流反馈能力，对应查询接口返回 `MOTOR_ERR_NOT_SUPPORTED`。
- 通用运行时间统计（`motor_runtime`）：所有电机类型驱动在死区处理后以实际有效输出为判据累计运行时间，时间基准由板级 `get_time_ms` 回调提供，通过 `Motor_SetRunningTime` / `Motor_GetRunningTime` 访问。

### 尚未完成

- 三个实际电机的板级实例化和统一初始化（含 PWM/GPIO/霍尔/电流及 `get_time_ms` 回调绑定）。
- 门锁四路开关的完整配置、去抖和时序逻辑。
...
`motor_driver` 为上层提供统一接口。当前输出量统一为 `0~1000`，它表示驱动输出请求，而不等同于实际闭环速度。测量位置、速度、电流等能力由具体电机实现按硬件条件提供，硬件不具备对应反馈的电机类型（如 VNH 门锁电机）将返回 `MOTOR_ERR_NOT_SUPPORTED`。运行时间则对所有电机类型统一提供。
...
| `motor_bdc_vnh` | VNH 类 PWM + INA + INB 电机实现（无位置/速度/电流反馈） |
| `motor_runtime` | 各电机类型共用的运行时间统计内联实现 |
...
- 无反馈电机（VNH）的每次动作必须先清零运行时间，动作中检查运行时间是否超过可标定的上限，超时立即停止并上报故障。
