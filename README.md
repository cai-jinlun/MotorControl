...
控制板接入六路低有效开关：半锁 PF1、全锁 PF2、中控 PF3，以及 KEY1 PF13、KEY2 PF14、KEY3 PF15。六路均使用内部上拉。
...
- GPIO、ADC1、DMA、CAN1、SPI3、TIM1、TIM2、TIM4、TIM6 和 USART2 的基础配置。
...
- 六路外部输入模块：支持低有效稳定状态、10/20 ms 去抖及上升沿/下降沿事件消费。

### 尚未完成

- 门锁与按键事件到具体业务指令的映射及时序逻辑。
...
当前 `main` 完成外设、三个板级电机及六路外部输入的初始化后，由 TIM6 执行 1 ms 输入采样，并在裸机主循环中周期执行电机超时服务，尚未运行完整的车门业务逻辑。
...
`external_input` 管理 HALF、FULL、CENTRAL 和三个按键。模块采用低有效逻辑，分别使用
10 ms 门锁去抖和 20 ms 按键去抖，并提供稳定电平、逻辑有效状态以及去抖后的电气
上升沿/下降沿位图。初始化只建立当前状态，不产生虚假的上电边沿。

边沿发布采用单生产者、单消费者模型。裸机阶段由 TIM6 ISR 每 1 ms 调用
`ExternalInput_Service(1U)`，预留主循环中的车门逻辑作为唯一消费者；加入 FreeRTOS 后停止 TIM6 采样，改由
传感器任务每 5 ms 调用 `ExternalInput_Service(5U)`，车门控制任务作为唯一边沿消费者。
核心模块不依赖 TIM6、HAL tick 或 FreeRTOS，也不会直接执行任何业务指令。
...
- 门锁机构负责协调两个门锁电机及半锁、全锁、中控三路信号，向上提供解锁、复位、吸合及锁状态查询。
...
- 传感器处理任务：每 5 ms 调用外部输入服务，并处理电流、霍尔和状态融合；产生边沿时通知车门控制任务。
...
| `external_input` | 六路低有效开关的毫秒去抖、稳定状态及边沿事件 |
| `board_motor_link` | H1/H4/H5 板级资源绑定、业务电机实例和统一初始化接口 |
